...
def create_databases():
    ...
    with sqlite3.connect("quotes_without_tags.db") as conn:
        conn.execute("""  CREATE TABLE IF NOT EXISTS quotes_without_tags (
            quote TEXT NOT NULL UNIQUE,
            author TEXT NOT NULL
        )
        """)

def extract_data_from_tagged_db(tags=[]):
    with sqlite3.connect("quotes_with_tags.db") as conn:  # connect to database and automatically close connection
        ...
        if tags==[]:
            ...
            cur.execute(f"SELECT quote, author, tags FROM quotes_with_tags WHERE {" OR ".join(["tags LIKE ?"] * len(tags))}", tags_in_query)
        return cur.fetchall()
...
def add_data_to_databases():
        ...
        with sqlite3.connect("quotes_with_tags.db") as conn:  # connect to database and automatically close connection
            ...
            next_page="/"
            while next_page:  #there are not so many quotes on site so can extract them all
